...
def flat_ts(n, genotypes):
    """
    A star tree over n samples with one site per column of genotypes, ancestral
    state "A" and a mutation wherever the genotype is not "A".
    """
    L = len(genotypes[0]) + 1
    tables = tskit.TableCollection(L)
    root = n
    for j in range(n):
        ...
        tables.edges.add_row(0, L, root, u)
    tables.nodes.add_row(time=1)
    for site in range(len(genotypes[0])):
        tables.sites.add_row(site, "A")
        for j in range(n):
            if genotypes[j][site] != "A":
                tables.mutations.add_row(
                    site=site, node=j, derived_state=genotypes[j][site]
                )
    tables.sort()
    return tables.tree_sequence()


class TestInferBinaryTopology:
    def test_single_sample(self):
        ts = flat_ts(1, ["T"])
        assert tree_ops.infer_binary_topology(ts) is None

    @pytest.mark.parametrize("n", [2, 3, 4, 8])
    def test_outgroup_is_root(self, n):
        # Each sample gets its own private mutation.
        genotypes = ["A" * j + "T" + "A" * (n - j - 1) for j in range(n)]
        ts = flat_ts(n, genotypes)
        pi, tau = tree_ops.infer_binary_topology(ts)
        assert len(tau) == len(pi)
        # The n samples and the outgroup are all leaves, so there must be
        # internal nodes on top of them.
        assert len(pi) > n + 1
        # Node n is the outgroup, and after rerooting it is the sole root.
        assert pi[n] == -1
        assert tau[n] == 1
        assert sum(p == -1 for p in pi) == 1
        # Every sample is a leaf, i.e. nothing points at it.
        assert set(pi) & set(range(n)) == set()

    def test_supplied_topology_is_used(self):
        # Two well separated pairs, so the topology is unambiguous.
        genotypes = ["TTAAAA", "TTAAAT", "AAATTA", "AATTTA"]
        ts1 = flat_ts(4, genotypes)
        topology = tree_ops.infer_binary_topology(ts1)
        expected = tree_ops.infer_binary(ts1, topology)

        # A different set of mutations that would give a different topology if
        # it were inferred rather than supplied.
        ts2 = flat_ts(4, ["TAAAAA", "AATAAA", "AAAATA", "AAAAAT"])
        inferred = tree_ops.infer_binary(ts2)
        supplied = tree_ops.infer_binary(ts2, topology)

        assert supplied.tables.edges == expected.tables.edges
        assert inferred.tables.edges != expected.tables.edges
        # The mutations still come from ts2, not from ts1.
        assert_sequences_equal(ts2, supplied)
